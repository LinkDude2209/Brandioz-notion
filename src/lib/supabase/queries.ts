...
export const getUserSubscriptionStatus = async (userId: string) => {
    try {
        ...
        if (data)
            return { data: data as Subscription, errror: null }
...
export const getFolders = async (workspaceId: string) => {
    ...
    try {
        const results: Folder[] | [] = await db.select().from(folders).orderBy(folders.createdAt)
...
export const getPrivateWorkspaces = async (userId: string) => {
    ...
    const privateWorkspaces = (await db
        ...
        .from(workspaces).where(and(notExists(db.select()
            .from(collaborators).where(eq(collaborators.workspaceId, workspaces.id)),
        ),
...
export const addCollaborators = async (users: User[], workspaceId: string) => {
    const response = users.forEach(async (user: User) => {
        const userExists = await db.select().from(collaborators).where(and(eq(collaborators.userId, user.id), eq(collaborators.workspaceId, workspaceId)));
        if (!userExists)
